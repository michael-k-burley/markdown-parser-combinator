...
""". stripMargin.replaceFirst("\n", "")
    ...
    */
    private def wrapInHTMLtags(element: Markdown): String = 
        ...
        wrapped_element ++= { 
            ...
            element match { 
                ...
                case List(value, isOrdered) => { 
                    ...
                    checkForSpanTags(value, if(isOrdered) then "ol" else "ul", f)
                }
    ...
    */
    def checkForSpanTags(md_lines: Vector[String], tag: String, f: Function1[String, String]): String = 

        val listFormating = Vector("ul", "ol").contains(tag) // Spacing to help with formating output
        val result: StringBuilder = new StringBuilder(s"<$tag>" + { if(listFormating) "\n" else "" }) 
            ...
            // Special formatting for list
            result ++= { if(listFormating) "\t\t" else "" }
